...
function buildProjectCardHTML(p, index) {
  ...
  const linkHTML = links.length
    ? links.join('')
    : `<span class="link-arrow" style="color:#6C5C86;"><i class="fa-solid fa-swatchbook"></i> No Live Link</span>`;
...
function renderCustomEducation() {
  ...
  if (certList) {
    ...
    const langRow = certList.querySelector('.languages-row');
    certs.forEach(cert => {
      const media = cert.image
        ? `<img class="cert-thumb" src="${cert.image}" alt="${escapeHTML(cert.title)} certificate">`
      ...
      if (langRow) langRow.insertAdjacentHTML('beforebegin', html);
      else certList.insertAdjacentHTML('beforeend', html);
    });
  }
}
